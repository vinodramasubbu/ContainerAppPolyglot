...
export class BatchingReceiver extends MessageReceiver {
    ...
    async receive(maxMessageCount, maxWaitTimeInMs, maxTimeAfterFirstMessageInMs, options) {
        ...
        try {
            logger.verbose("[%s] Receiver '%s', setting max concurrent calls to 0.", this.logPrefix, this.name);
...
export class BatchingReceiverLite {
    ...
    _receiveMessagesImpl(receiver, args, origResolve, origReject) {
        ...
        const onReceiveMessage = async (context) => {
            ...
            try {
                ...
                if (brokeredMessages.length > args.maxMessageCount) {
                    logger.warning(`More messages arrived than were expected: ${args.maxMessageCount} vs ${brokeredMessages.length + 1}`);
                }
            }
            catch (err) {
        ...
        const onReceiveDrain = () => {
            ...
            logger.verbose(`${loggingPrefix} Drained, resolving receiveMessages() with ${brokeredMessages.length} messages.`);
        ...
        logger.verbose(`${loggingPrefix} Adding credit for receiving ${args.maxMessageCount} messages.`);
